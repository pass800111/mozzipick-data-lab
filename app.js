...
function draw(list=products){ranking.innerHTML=list.map((p,i)=>`<article class="product" data-i="${products.indexOf(p)}"><div class="rank ${i<2?"top":""}">${i+1}</div><div class="thumb">PRODUCT</div><div><div class="name">${p[0]}</div><div class="meta">${p[1]} · ${p[2]} · ${p[3]}</div><div class="status">${p[8].map(x=>`<span class="tag">${x}</span>`).join("")}<span class="tag ${p[9]==="verified"?"new":""}">${p[6]}</span></div></div><div class="mini">${bars(p[4])}</div><div class="trend ${p[9]==="verified"?"up":""}">${p[5]}<b>${p[7]}</b></div></article>`).join("");document.querySelectorAll(".product").forEach(x=>x.onclick=()=>openDetail(+x.dataset.i))}
draw();
