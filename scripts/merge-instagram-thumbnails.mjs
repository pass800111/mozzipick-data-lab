...
function imageOf(p){return [p.thumbnail,p.thumbnailUrl,p.displayUrl,p.videoThumbnail,p.coverUrl,p.cover,p.imageUrl,p.previewUrl].find(v=>typeof v==='string'&&/^https:\/\//i.test(v.trim())&&!/\s/.test(v))||''}
const from=new Map();for(const p of records){const k=key(p.reel||p.url||p.postUrl||p.permalink||p.inputUrl||p.shortCode||p.shortcode);const v=imageOf(p);if(k&&v)from.set(k,{image:v,source:p.reel||p.url||p.postUrl||p.permalink||p.inputUrl||''})}
